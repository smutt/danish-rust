...
// RFC 6606 Sec 3 says, "DNS hostnames are case-insensitive.", but we only check lowercase. Haven't yet found an instance where this matters.
// Can investigate use of --icase if necessary
// TODO: Handle errors generated by this function
fn ipt_add_long(ipt: &iptables::IPTables, chain: &String, sni: &String) -> Result<(), iptables::error::IPTError> {
    if !sni.is_ascii(){
        error!("SNI not ascii {}", &sni);
        return Err(iptables::error::IPTError::Other("SNI not ascii"));
    }
    // format => 0x0000 + entire_len(2 bytes) + extension_len(2 bytes) + 0x00 + sni_len(2 bytes) + sni
    let hex_sni: String = sni.as_bytes().iter().map(|b| format!("{:X}", b)).collect();
    let sni_len: String = format!("{:01$x}", sni.len(), 4);
...
fn main() {
    ...
    if cli_opts.chain.to_uppercase() != "OUTPUT".to_string() && cli_opts.chain.to_uppercase() != "FORWARD".to_string() {
        cease(&format!("Invalid iptables/ip6tables chain {:?}", cli_opts.chain.to_uppercase()));
    }
    if cli_opts.sub_chain.len() > IPT_SUBCHAIN_MAX_CHARS { 
        cease(&format!("sub-chain argument too long {:?}", cli_opts.sub_chain));
    }
    if cli_opts.sub_chain.len() < IPT_SUBCHAIN_MIN_CHARS {
        cease(&format!("sub-chain argument too short {:?}", cli_opts.sub_chain));
    }
    if !cli_opts.sub_chain.is_ascii() {
        cease(&format!("non-ASCII character in sub-chain"));
    }
    for cc in cli_opts.sub_chain.chars() {
        if !cc.is_ascii_alphanumeric() {
            cease(&format!("Invalid ASCII character in sub-chain"));
        }
    }
    ...
    match iptables::new(false) {
        ...
        Ok(ipt) => {
            ...
            if ipv6_enabled() {
                match iptables::new(true) {
                    Err(_) => cease(&(format!("FATAL ip6tables error"))),
                    Ok(ipt6) => {
